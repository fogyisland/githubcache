 ...
 *
 * Preview what would happen if we ran the provider now. Returns the
 * preview totals (items / urls / unique / existing / stale / new / invalid)
 * plus a sample of up to 20 owner/name pairs that would be queued.
 *
 * Body: `{ limit?: number, csrf }`
 *
 * Admin or operator may preview (read-only, no DB writes).
 */
const Body = z.object({
...
export async function POST(
  ...
  try {
    ...
    logger.error({ err: e, providerId: id }, 'preview provider failed');
    return apiError('internal_error', 'preview failed', {}, req);
  }
}